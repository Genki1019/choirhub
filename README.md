...
> デモアカウントは管理者（`admin` + `conductor`）権限です（デモ団体の削除のみ無効化しています）。他の団員アカウント（例: `sakura@harmonia.example`）でも同パスワードでログインし、一般団員視点の画面を確認できます。
...
誤操作を防ぐため、団体名の入力とパスワードの再認証を求めたうえで論理削除し、30日間は運営への問い合わせで復元できます。

| 団体削除の確認                                    | お問い合わせ                             |
| ------------------------------------------------- | ---------------------------------------- |
| ![団体削除の確認](docs/images/setting_delete.png) | ![お問い合わせ](docs/images/contact.png) |
...
| **設定**               | パート・会費方式のカスタマイズ・団体削除（30日間は復元可、以降自動で完全削除）           |
| **運営窓口**           | 運営への問い合わせフォーム（管理者コンソールで対応管理）・利用規約・プライバシーポリシー |
...
│   │   │   ├── (info)/       # 公開ページ（contact / privacy / terms）
│   │   │   ├── admin/        # システム管理者コンソール
│   │   │   └── [org]/        # テナント別ルート
│   │   └── lib/              # API クライアント・型定義
│   └── api/                  # バックエンド API（Hono）
...
ブラウザ
  ...
  └─→ /api/v1/*  ──────────→ Hono API (Vercel)
                                  ...
                                  └── Vercel Cron → 定期バッチ（出欠期限リマインド・既読通知の自動削除・削除済み団体の完全削除）
```
...
- 団体の削除は **論理削除 + 30 日間の猶予期間** 方式。削除した時点でテナント解決の入口（`tenant.ts`・iCal フィード・Webhook・招待）で遮断し、猶予期間中はシステム管理者が復元可能。期限後は日次 Cron が R2 のファイルと DB（`onDelete: Cascade`）を完全削除する
...
- 上記とは別に、環境変数 `SYSTEM_ADMIN_EMAILS` で指定する **システム管理者** が団体作成申請の承認・削除済み団体の復元・問い合わせ対応を担う（団体の運営には関与しない）
...
| `SYSTEM_ADMIN_EMAILS`      |  -   | システム管理者（団体作成申請の承認・削除済み団体の復元・問い合わせの通知先）のメールアドレス（カンマ区切り） |
| `CRON_SECRET`              |  -   | Vercel Cron バッチ（出欠期限通知・既読通知の自動削除・削除済み団体の完全削除）の認証トークン                 |
| `PROTECTED_ORG_SLUGS`      |  -   | 削除を禁止する団体のスラグ（カンマ区切り）。公開デモ団体（`harmonia`）を指定する                             |

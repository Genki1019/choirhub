...
async function seedAdmin() {
  ...
  if (!existing) {
    await prisma.member.create({ data: { userId: user.id, orgId: org.id, roles: ["admin"], status: "active" } });
  }
...
async function seedDemo() {
  ...
  const [catRehearsal, , catMeeting] = await prisma.eventCategory.findMany({
    where: { orgId: org.id }, orderBy: { sortOrder: "asc" },
  });
  ...
  type MemberDef = {
    ...
    partId: string; memberTypeId: string; roles: string[];
    joinedAt: Date; status: "active" | "offstage";
  };

  const memberDefs: MemberDef[] = [
    { email: DEMO_EMAIL,                 nameJa: "山田 恵子", nameKana: "ヤマダ ケイコ",    partId: pAlto.id,    memberTypeId: mtGeneral.id, roles: ["admin", "conductor"], joinedAt: new Date("2018-04-01"), status: "active" },
    { email: "sakura@harmonia.example",  nameJa: "鈴木 さくら", nameKana: "スズキ サクラ",   partId: pSoprano.id, memberTypeId: mtGeneral.id, roles: ["member", "score"],     joinedAt: new Date("2019-09-01"), status: "active" },
    { email: "hina@harmonia.example",    nameJa: "伊藤 ひな",  nameKana: "イトウ ヒナ",     partId: pSoprano.id, memberTypeId: mtStudent.id, roles: ["member"],              joinedAt: new Date("2024-04-01"), status: "active" },
    { email: "misaki@harmonia.example",  nameJa: "佐藤 美咲",  nameKana: "サトウ ミサキ",   partId: pSoprano.id, memberTypeId: mtGeneral.id, roles: ["member"],              joinedAt: new Date("2021-04-01"), status: "active" },
    { email: "nana@harmonia.example",    nameJa: "田中 なな",  nameKana: "タナカ ナナ",     partId: pAlto.id,    memberTypeId: mtGeneral.id, roles: ["member"],              joinedAt: new Date("2020-04-01"), status: "active" },
    { email: "mai@harmonia.example",     nameJa: "森 麻衣",   nameKana: "モリ マイ",       partId: pAlto.id,    memberTypeId: mtStudent.id, roles: ["member"],              joinedAt: new Date("2023-09-01"), status: "active" },
    { email: "taro@harmonia.example",    nameJa: "渡辺 太郎",  nameKana: "ワタナベ タロウ", partId: pTenor.id,   memberTypeId: mtGeneral.id, roles: ["member"],              joinedAt: new Date("2019-04-01"), status: "active" },
    { email: "kenji@harmonia.example",   nameJa: "小林 健二",  nameKana: "コバヤシ ケンジ", partId: pTenor.id,   memberTypeId: mtStudent.id, roles: ["member"],              joinedAt: new Date("2025-04-01"), status: "active" },
    { email: "daisuke@harmonia.example", nameJa: "加藤 大輔",  nameKana: "カトウ ダイスケ", partId: pBass.id,    memberTypeId: mtGeneral.id, roles: ["member", "finance"],   joinedAt: new Date("2018-04-01"), status: "active" },
    { email: "makoto@harmonia.example",  nameJa: "松本 誠",   nameKana: "マツモト マコト",  partId: pBass.id,    memberTypeId: mtGeneral.id, roles: ["member"],              joinedAt: new Date("2022-07-01"), status: "active" },
  ];
  ...
  for (const def of memberDefs) {
    ...
    const member = await prisma.member.create({
      data: {
        ...
        partId: def.partId, memberTypeId: def.memberTypeId,
        roles: def.roles, status: def.status, joinedAt: def.joinedAt,
      },
    });
  ...
  const scores = await Promise.all([
    prisma.score.create({ data: {
      orgId: org.id, title: "ハレルヤ", composer: "G. F. Handel", arranger: "合唱編曲版",
      accessLevel: "restricted", distributionPrice: 300, notes: "Messiah HWV 56 より",
      purchaseDate: new Date("2025-06-01"),
    }}),
    prisma.score.create({ data: {
      orgId: org.id, title: "アヴェ・ヴェルム・コルプス", composer: "W. A. Mozart",
      accessLevel: "restricted", distributionPrice: 200, notes: "K. 618",
      purchaseDate: new Date("2025-06-01"),
    }}),
    prisma.score.create({ data: {
      orgId: org.id, title: "主よ、人の望みの喜びよ", composer: "J. S. Bach", arranger: "合唱編曲版",
      accessLevel: "public", notes: "BWV 147 より",
    }}),
    prisma.score.create({ data: {
      orgId: org.id, title: "歓喜の歌", composer: "L. v. Beethoven", arranger: "合唱編曲版",
      accessLevel: "public", notes: "交響曲第9番 第4楽章より",
    }}),
    prisma.score.create({ data: {
      orgId: org.id, title: "ひかりのうた", composer: "田中 恵一",
      accessLevel: "secret", isCommissioned: true, notes: "第16回定期演奏会 委嘱作品",
    }}),
  ]);
  ...
  const pastConcert = await prisma.concert.create({
    data: {
      orgId: org.id, title: "第15回定期演奏会",
      heldOn: pastConcertDate, venue: "東京文化会館 小ホール",
      status: "past", ticketInputClosedAt: addDays(pastConcertDate, 7),
    },
  });
  ...
  const nextConcert = await prisma.concert.create({
    data: {
      orgId: org.id, title: "第16回定期演奏会",
      heldOn: nextConcertDate, venue: "横浜みなとみらいホール 小ホール",
      status: "survey_open", outreachExpensePerTrip: 500,
    },
  });
  const st16a = await prisma.stage.create({ data: { concertId: nextConcert.id, name: "第1ステージ", sortOrder: 1 } });
  const st16b = await prisma.stage.create({ data: { concertId: nextConcert.id, name: "第2ステージ", sortOrder: 2 } });
  await Promise.all([
    prisma.program.create({ data: { stageId: st16a.id, scoreId: sAveVerum.id,   title: sAveVerum.title,   sortOrder: 1 } }),
    prisma.program.create({ data: { stageId: st16a.id, scoreId: sHikari.id,     title: sHikari.title,     sortOrder: 2 } }),
    prisma.program.create({ data: { stageId: st16b.id, scoreId: sHallelujah.id, title: sHallelujah.title, sortOrder: 1 } }),
    prisma.program.create({ data: { stageId: st16b.id, scoreId: sOdeToJoy.id,   title: sOdeToJoy.title,   sortOrder: 2 } }),
  ]);

  // 第16回 オンステ調査
  const survey = await prisma.concertSurvey.create({
    data: {
      concertId: nextConcert.id, title: "第16回定期演奏会 オンステ調査",
      openAt: addDays(today, -7), closeAt: addDays(nextConcertDate, -60), isOpen: true,
    },
  });
  ...
  for (let mi = 0; mi < members.length; mi++) {
    for (const [si, stage] of [[0, st16a], [1, st16b]] as [number, { id: string }][]) {
      await prisma.surveyResponse.create({
        data: {
          surveyId: survey.id, memberId: members[mi].id,
          stageId: stage.id, status: surveyPatterns[mi][si],
        },
      });
    }
  }
  ...
  for (const m of members) {
    await prisma.scorePurchase.create({
      data: { scoreId: sHallelujah.id, memberId: m.id, amount: 300, purchasedAt: buyDate15, recordedById: adminMemberId },
    });
  }
  // アヴェ・ヴェルム: 8名購入（後半2名未購入）
  for (const m of members.slice(0, 8)) {
    await prisma.scorePurchase.create({
      data: { scoreId: sAveVerum.id, memberId: m.id, amount: 200, purchasedAt: buyDate15, recordedById: adminMemberId },
    });
  }
  ...
  const batch15 = await prisma.ticketBatch.create({
    data: { concertId: pastConcert.id, name: "一般", price: 2000, priceStudent: 1000, totalCount: 200 },
  });
  const soldAmounts = [15, 8, 6, 12, 9, 4, 7, 5, 11, 8];
  for (let i = 0; i < members.length; i++) {
    await prisma.ticketAllocation.create({
      data: {
        batchId: batch15.id, memberId: members[i].id,
        allocatedCount: 20, soldAdult: soldAmounts[i],
        soldStudent: Math.floor(soldAmounts[i] / 4),
        isCollected: true, reportedAt: pastConcertDate,
      },
    });
  }
  ...
  const batch16 = await prisma.ticketBatch.create({
    data: { concertId: nextConcert.id, name: "一般", price: 2500, priceStudent: 1500, totalCount: 200 },
  });
  for (const m of members) {
    await prisma.ticketAllocation.create({
      data: {
        batchId: batch16.id, memberId: m.id,
        allocatedCount: 10, requestedCount: 10,
        soldAdult: 0, isCollected: false,
      },
    });
  }
  ...
  const expenseDefs = [
    { categoryId: ecVenue.id,  title: "4月練習スタジオ代",   amount: 8000, paidAt: addDays(today, -77), paymentMethod: "cash" as const },
    { categoryId: ecVenue.id,  title: "5月練習スタジオ代",   amount: 8000, paidAt: addDays(today, -47), paymentMethod: "cash" as const },
    { categoryId: ecVenue.id,  title: "6月練習スタジオ代",   amount: 8000, paidAt: addDays(today, -14), paymentMethod: "cash" as const },
    { categoryId: ecPrint.id,  title: "ハレルヤ楽譜印刷費",  amount: 3500, paidAt: addDays(today, -80), paymentMethod: "bank_transfer" as const },
    { categoryId: ecEquip.id,  title: "指揮棒・文具一式",    amount: 1200, paidAt: addDays(today, -60), paymentMethod: "cash" as const },
    { categoryId: ecOther.id,  title: "打ち上げ補助",        amount: 5000, paidAt: addDays(today, -86), paymentMethod: "cash" as const },
  ];
  ...
  for (const def of collDefs) {
    const coll = await prisma.collection.create({
      data: {
        orgId: org.id, title: def.title, amount: def.amount,
        yearMonth: def.yearMonth, createdById: adminMemberId,
      },
    });
    for (let i = 0; i < members.length; i++) {
      ...
      await prisma.collectionPayment.create({
        data: {
          collectionId: coll.id, memberId: members[i].id,
  ...
  const mailDefs = [
    {
      ...
      bodyPreview: "5月の練習は毎週日曜日13:00〜17:00です。場所はいつものスタジオです。出欠の入力をお忘れなくお願いします。",
      sentAt: addDays(today, -42),
    },
    {
      subject:     "【ハーモニア】第16回定期演奏会 日程確定のお知らせ",
      bodyPreview: "第16回定期演奏会の日程が確定しました。会場は横浜みなとみらいホール小ホールです。オンステ調査を本日より開始しますので、ご回答をお願いします。",
      sentAt: addDays(today, -14),
    },
    {
      subject:     "【ハーモニア】楽譜配布のお知らせ（ハレルヤ・アヴェヴェルム）",
      bodyPreview: "第16回に向けて「ハレルヤ」「アヴェ・ヴェルム・コルプス」の楽譜をシステムにアップしました。購入記録のある方はダウンロードページよりお取りください。",
  ...
  for (const def of mailDefs) {
    await prisma.mailLog.create({
      data: {
        orgId: org.id, sentById: adminMemberId,
        subject: def.subject, bodyPreview: def.bodyPreview,
        sentAt: def.sentAt, recipientMemberIds: members.map((m) => m.id),
      },
    });
  }

...
describe("GET /auth/me", () => {
  ...
  it("システム管理者としてログイン中: isSystemAdmin が true になる", async () => {
    const originalEnv = process.env.SYSTEM_ADMIN_EMAILS;
    process.env.SYSTEM_ADMIN_EMAILS = testUser.email;
    ...
    const body = await json(res);
    expect(body.data.user.isSystemAdmin).toBe(true);

    process.env.SYSTEM_ADMIN_EMAILS = originalEnv;
  });
});

...
export const concertsRouter = new Hono<TenantEnv>()
  ...
  .post(
    ...
    async (c) => {
      ...
      if (!isAdmin(actingMember)) {
        return c.json(
          { error: { code: "FORBIDDEN", message: "管理者のみステージを追加できます" } },
          403,
        );
      }
      ...
      const maxStageOrder = await prisma.stage.aggregate({
        where: { concertId },
        _max: { sortOrder: true },
      });
      const stageSortOrder = (maxStageOrder._max.sortOrder ?? 0) + 1;
      const stage = await prisma.stage.create({
        data: { concertId, name, sortOrder: stageSortOrder },
      });
        ...
        return c.json(
          {
            error: {
              code: "BAD_REQUEST",
              message: "このコンサートに属さないステージが含まれています",
            },
          },
          400,
        );
  ...
  .put(
    ...
    async (c) => {
      ...
      if (ids.some((id) => !validProgramIds.has(id))) {
        return c.json(
          { error: { code: "BAD_REQUEST", message: "このステージに属さない演目が含まれています" } },
          400,
        );
      }
  ...
  .post(
    ...
    async (c) => {
      ...
      if (!scoreId && !title?.trim()) {
        return c.json(
          { error: { code: "VALIDATION_ERROR", message: "曲名またはscoreIdが必要です" } },
          400,
        );
      }
      ...
      if (scoreId) {
        ...
        const maxOrder1 = await prisma.program.aggregate({
          where: { stageId },
          _max: { sortOrder: true },
        });
        ...
        const maxOrder2 = await prisma.program.aggregate({
          where: { stageId },
          _max: { sortOrder: true },
        });
        ...
        program = await prisma.program.create({
          data: { stageId, scoreId: newScore.id, title: trimmedTitle, sortOrder: sortOrder2 },
        });
        responseScore = {
          id: newScore.id,
  ...
  .delete("/concerts/:concertId/programs/:programId", async (c) => {
    const actingMember = c.get("member");
    const org = c.get("org");
  ...
  .get("/concerts/:id", async (c) => {
        ...
        data: {
          ...
          stages: concert.stages.map((stage) => ({
            ...
            programs: stage.programs.map((p) => ({
              ...
              score: p.score
                ? { id: p.score.id, composer: p.score.composer, arranger: p.score.arranger }
                : null,
            })),
          })),
          surveys: [],
    ...
    const assignments = concert.onStageAssignments
  ...
  .post(
    ...
    async (c) => {
      ...
      await prisma.concertSurvey.updateMany({
  ...
  .get("/concerts/:concertId/surveys/:surveyId", async (c) => {
    ...
    const responseMap = new Map<
      string,
      Map<string, { status: "attending" | "absent" | "undecided"; memo: string | null }>
    >();
  ...
  .patch(
    ...
    async (c) => {
      ...
      if (body.isOpen === true) {
        ...
        const stillOpen = await prisma.concertSurvey.count({
          where: { concertId, isOpen: true, id: { not: surveyId } },
        });
        if (stillOpen === 0) {
      ...
      return c.json({
        data: { id: updated.id, title: updated.title, isOpen: updated.isOpen, concertStatus },
      });
    },
  )
  ...
  .put(
    ...
    async (c) => {
      ...
      if (targetMemberId && targetMemberId !== actingMember.id && !isAdmin(actingMember)) {
        return c.json(
          { error: { code: "FORBIDDEN", message: "管理者のみ他のメンバーの回答を変更できます" } },
          403,
        );
      }
      ...
      if (targetMemberId && targetMemberId !== actingMember.id) {
        ...
        if (!target || target.orgId !== org.id) {
          return c.json(
            { error: { code: "NOT_FOUND", message: "指定されたメンバーが見つかりません" } },
            404,
          );
        }
      }
      ...
      if (invalidStage) {
        return c.json(
          { error: { code: "NOT_FOUND", message: "無効なステージIDが含まれています" } },
          404,
        );
      }
      ...
      if (!survey.isOpen) {
        await syncOnStageFromResponses(
          concertId,
          responses.map((r) => ({ memberId, stageId: r.stageId, status: r.status })),
        );
      }

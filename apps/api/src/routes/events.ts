...
function isInvited(member: Member, event: Event): boolean {
  const roleMatch =
    ...
    event.targetRoles.some((r) => member.roles.includes(r));
...
function formatEvent(event: Event & { concertId?: string | null; category: EventCategory }) {
  return {
    id:            event.id,
    title:         event.title,
    category:      { id: event.category.id, name: event.category.name, slug: event.category.slug, color: event.category.color },
...
export const eventsRouter = new Hono<TenantEnv>()
  ...
  .get("/events", async (c) => {
    ...
    if (from && isNaN(Date.parse(from))) {
      return c.json({ error: { code: "VALIDATION_ERROR", message: "from パラメータの日付形式が不正です" } }, 400);
    }
    if (to && isNaN(Date.parse(to))) {
      return c.json({ error: { code: "VALIDATION_ERROR", message: "to パラメータの日付形式が不正です" } }, 400);
    }
    ...
    const visible = isAdmin(member)
      ? events
      : events.filter((ev) => isInvited(member, ev));
    ...
    if (includeConcerts) {
      ...
      const catFallback = concertCategory ?? { id: "", name: "本番", slug: "concert", color: "#F97316" };

      for (const ct of concerts) {
        concertItems.push({
          id:            ct.id,
          title:         ct.title,
          category:      { id: catFallback.id, name: catFallback.name, slug: catFallback.slug, color: catFallback.color },
  ...
  .post("/events", zValidator("json", eventBodySchema), async (c) => {
    ...
    const selectedCategory = await prisma.eventCategory.findUnique({ where: { id: body.categoryId } });
  ...
  .get("/events/:id", async (c) => {
    ...
    if (!isAdmin(member) && !isInvited(member, event)) {
      return c.json({ error: { code: "NOT_INVITED", message: "このイベントへの参加権限がありません" } }, 403);
    }

    const invitedAttendances = (isAdmin(member)
      ? event.attendances
    ...
    invitedAttendances.forEach((a) => { summary[a.status]++; });
      ...
      data: {
        ...
        attendances: invitedAttendances.map((a) => ({
          member: {
            ...
            part:   a.member.part
              ? { id: a.member.part.id, name: a.member.part.name, sortOrder: a.member.part.sortOrder, voiceType: a.member.part.voiceType }
              : null,
          },
          status:     a.status,
  ...
  .patch("/events/:id/attendance/:memberId", zValidator("json", attendanceBodySchema.partial()), async (c) => {
    const org    = c.get("org");
    const member = c.get("member");

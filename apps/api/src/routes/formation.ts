...
export const formationRouter = new Hono<TenantEnv>()
  ...
  .post(
    ...
    async (c) => {
      ...
      const maxOrder = await prisma.formationPattern.aggregate({
        where: { stageId },
        _max: { sortOrder: true },
      });
  ...
  .patch(
    ...
    async (c) => {
      ...
      const updated = await prisma.formationPattern.update({
        where: { id: patternId },
        data: { name, isStaggered, pianoPosition },
      });
      ...
      return c.json({
        data: {
          ...
          isStaggered: updated.isStaggered,
          pianoPosition: updated.pianoPosition,
        },
      });
    },
  )
  ...
  .put(
    ...
    async (c) => {
      ...
      const patterns = await prisma.formationPattern.findMany({
        where: { stageId },
        select: { id: true },
      });
        ...
        return c.json(
          {
            error: {
              code: "BAD_REQUEST",
              message: "このステージに属さないパターンが含まれています",
            },
          },
          400,
        );
      ...
      await Promise.all(
        ids.map((id, i) =>
          prisma.formationPattern.update({ where: { id }, data: { sortOrder: i + 1 } }),
        ),
      );
    ...
    async (c) => {
      ...
      if (memberIds.length > 0) {
        const members = await prisma.member.findMany({
          where: { orgId: org.id, id: { in: memberIds } },
          select: { id: true },
        });
        ...
        if (memberIds.some((id) => !validMemberIds.has(id))) {
          return c.json(
            {
              error: { code: "BAD_REQUEST", message: "この団体に属さないメンバーが含まれています" },
            },
            400,
          );
        }
          ...
          return c.json(
            {
              error: {
                code: "BAD_REQUEST",
                message: "このステージでオンステ確定していないメンバーが含まれています",
              },
            },
            400,
          );
      ...
      if (slots.some((s) => s.boxClientId != null && !boxClientIds.has(s.boxClientId))) {
        return c.json(
          {
            error: { code: "BAD_REQUEST", message: "存在しない枠を参照しているスロットがあります" },
          },
          400,
        );
      }

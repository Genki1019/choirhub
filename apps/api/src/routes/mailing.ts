...
export const mailingRouter = new Hono<TenantEnv>()
  ...
  .get("/mailing", async (c) => {
    ...
    const where = {
      ...
      OR: [
        { sentById: member.id },
        { recipientMemberIds: { has: member.id } },
      ],
    };
    ...
    const [total, mails] = await Promise.all([
      ...
      prisma.mailLog.findMany({
        ...
        skip:    (page - 1) * perPage,
        take:    perPage,
        include: { sentBy: { include: { userRef: { select: { nameJa: true, avatarUrl: true } } } } },
      }),
    ]);
  ...
  .post(
    ...
    async (c) => {
      ...
      return c.json({ data: { id: template.id, name: template.name, subject: template.subject, body: template.body, updatedAt: template.updatedAt.toISOString() } }, 201);
    },
  )
  ...
  .patch(
    ...
    async (c) => {
      ...
      if (!template) return c.json({ error: { code: "NOT_FOUND", message: "テンプレートが見つかりません" } }, 404);
      if (!isAdmin(member) && template.createdById !== member.id) return c.json({ error: { code: "FORBIDDEN", message: "権限がありません" } }, 403);
      const updated = await prisma.mailTemplate.update({ where: { id }, data: c.req.valid("json") });
      return c.json({ data: { id: updated.id, name: updated.name, subject: updated.subject, body: updated.body, updatedAt: updated.updatedAt.toISOString() } });
    },
  )
  ...
  .get("/mailing/:id", async (c) => {
    ...
    const resendDetails = await Promise.all(
      mail.resendIds.map((rid) => getResendEmail(rid))
    );
  ...
  .post(
    ...
    async (c) => {
      ...
      if (!isMemberPlus(actingMember)) {
        return c.json({ error: { code: "FORBIDDEN", message: "メール送信は一般団員以上のみ可能です" } }, 403);
      }
      ...
      if (recipientType === "part"   && recipientFilter?.partIds?.length)   baseWhere.partId = { in: recipientFilter.partIds };
      else if (recipientType === "role"   && recipientFilter?.roles?.length)     baseWhere.roles  = { hasSome: recipientFilter.roles };
      else if (recipientType === "custom" && recipientFilter?.memberIds?.length) baseWhere.id     = { in: recipientFilter.memberIds };
      ...
      const resendIds            = await sendBulkMail({ to: emails.map((email) => ({ email })), subject, body, orgName: org.name });
      ...
      return c.json({
        data: { mailLogId: mailLog.id, recipientCount: emails.length, sentAt: mailLog.sentAt.toISOString() },
      }, 201);
    },
  );

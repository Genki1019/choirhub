...
export function EventCard({
  ...
  event: EventCardItem;
  org: string;
  from?: ConcertLinkSource;
}) {
  const status = ATTENDANCE_LABEL[event.myAttendance];
  const href = event.concertId
    ? getConcertHref(org, event.concertId, from)
    : `/${org}/schedule/${event.id}`;

  return (
    <Link
      href={href}
      prefetch={false}
      className="flex overflow-hidden rounded-xl border border-gray-200 bg-white transition-shadow hover:shadow-sm"
    >
      <div className="w-1 shrink-0" style={{ backgroundColor: event.category.color }} />
      <div className="flex-1 px-5 py-4">
        <p className="mb-1 text-xs text-gray-400">{formatEventDate(event.startsAt)}</p>
        ...
        <div className={`mt-2 flex items-center gap-1.5 text-xs font-medium ${status.className}`}>

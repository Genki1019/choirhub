...
export function FormationEditor({
  ...
  partColorMap,
  onFormationChanged,
  onIsStaggeredChanged,
  onPianoPositionChanged,
}: {
  org: string;
  concertId: string;
  stageId: string;
  ...
  const [memberLabels, setMemberLabels] = useState<Record<string, string>>(() => {
    ...
    pattern.slots.forEach((s) => {
      if (s.memberId && s.label) map[s.memberId] = s.label;
    });
    return map;
  });
  ...
  const commit = (nextPlaced: Containers, nextBoxes: BoxMeta[] = boxes) => {
    ...
    concertsApi
      .saveFormationSlots(
        ...
        stageId,
        pattern.id,
        buildSlotsPayload(nextPlaced, nextBoxes),
      )
      .then(() => {
  ...
  const placeMemberInBox = (boxKey: string, member: AssignmentDetail) => {
    const item: SlotItem = {
      ...
      label: memberLabels[member.memberId] ?? null,
      name: member.nameJa,
  ...
  const removeBox = (boxKey: string) => {
    ...
    commit(
      next,
      boxes.filter((b) => b.key !== boxKey),
    );
  };

  const openNameEdit = (item: SlotItem, e: React.MouseEvent) => {
    setNameEdit({
      target: { type: "item", key: item.key },
      x: e.clientX,
      y: e.clientY,
      value: displayLabelOf(item),
    });
  };
  ...
  const saveNameEdit = (value: string) => {
    ...
    if (nameEdit.target.type === "box") {
      ...
      commit(
        placed,
        boxes.map((b) => (b.key === boxKey ? { ...b, title: trimmed } : b)),
      );
      setNameEdit(null);
      return;
    }
    ...
    if (!rowKeyFound || !target) {
      setNameEdit(null);
      return;
    }
    if (!target.memberId && !trimmed) {
      setNameEdit(null);
      return;
    }
    if (target.memberId) {
      ...
      commit({
        ...placed,
        [rowKeyFound]: placed[rowKeyFound].map((i) =>
          i.key === itemKey ? { ...i, label: trimmed || null } : i,
        ),
      });
    } else {
      commit({
        ...placed,
        [rowKeyFound]: placed[rowKeyFound].map((i) =>
          i.key === itemKey ? { ...i, label: trimmed, name: trimmed } : i,
        ),
      });
    }
    setNameEdit(null);
  ...
  const removeLastRow = () => {
    ...
    if (placed[lastKey].length > 0) commit(next);
    else setPlaced(next);
  };
  ...
  type ItemLocation =
    { kind: "pool" } | { kind: "box"; key: string } | { kind: "grid"; row: string; col: number };
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    const sourceKey =
      source.kind === "grid" ? source.row : source.kind === "box" ? source.key : null;
    const draggedItem: SlotItem =
      source.kind === "pool"
        ? {
            ...unassignedMembers.find((i) => i.key === activeKey)!,
            key: `i:${crypto.randomUUID()}`,
          }
    ...
    if (overCell) {
      ...
      const occupant = (placed[targetRow] ?? []).find(
        (i) => i.col === targetCol && i.key !== activeKey,
      );
    ...
    const isOverUnassigned =
      overKey === "unassigned" || unassignedMembers.some((i) => i.key === overKey);
    const targetKey = isOverUnassigned
      ? "unassigned"
      : overKey in placed
        ? overKey
        : findContainer(placed, overKey);
    if (!targetKey) return;
  ...
  const maxUsedCol = Math.max(
    0,
    ...rowKeys.flatMap((k) => (placed[k] ?? []).map((i) => i.col ?? 0)),
  );
  ...
  const renderBox = (boxKey: string) => {
    ...
    return (
      <div key={boxKey} className={isFixed ? "min-w-0 flex-1" : "min-w-0"}>
          ...
          <div className="flex items-center gap-3">
            ...
            {!isFixed && (
              <>
                <button
                  type="button"
                  onClick={(e) => openBoxRename(boxKey, e)}
    ...
    >
      <div className="space-y-4">
        ...
        <div className="space-y-3 rounded-xl border border-gray-200 bg-gray-50/50 p-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <div
                ...
                {...addMemberPopover.containerProps}
              >
                <button
                  type="button"
                  {...addMemberPopover.triggerProps}
                  className="hover:text-brand-600 flex items-center gap-1 rounded-lg border border-gray-200 bg-white px-2 py-1 text-xs text-gray-500"
                >
              ...
              <div className="relative" ref={addBoxPopoverRef} {...addBoxPopover.containerProps}>
                ...
                {addBoxPopover.isOpen && <AddBoxPopover onCreateBox={createBox} />}
              </div>
            </div>
          </div>
          ...
          {customBoxes.length > 0 && (
            <div className="grid grid-cols-2 gap-3">{customBoxes.map((b) => renderBox(b.key))}</div>
          )}
        </div>
        ...
        <div className="space-y-3 rounded-xl border border-gray-200 bg-gray-50/50 p-3">
            ...
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-3">
                <label className="flex cursor-pointer items-center gap-1.5 text-xs text-gray-500 select-none">
                  ...
                  >
                    <span
                      className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${pattern.isStaggered ? "translate-x-4" : "translate-x-0.5"}`}
                    />
                  </button>
                  半人分ずらす
                </label>
                <div className="flex items-center gap-1">
                  <button
                    ...
                    className="rounded border border-gray-200 bg-white p-1 text-gray-400 hover:text-gray-600"
                    title="末尾の段を減らす"
                  >
                    <Minus size={12} />
                  </button>
                  <span className="w-6 text-center text-xs text-gray-400">{rowKeys.length}段</span>
                  <button
                    type="button"
                    onClick={addRow}
                    className="rounded border border-gray-200 bg-white p-1 text-gray-400 hover:text-gray-600"
              ...
              {rowKeys.map((row, idx) => {
                ...
                return (
                  <div key={row} className="flex items-center gap-2">
                    <span className="w-10 shrink-0 truncate text-xs text-gray-600">
                      {idx + 1}段目
                    </span>
                    <div className="flex flex-1 justify-center">
                      <div
                        ...
                        style={
                          isOffsetRow
                            ? { transform: `translateX(${GRID_STAGGER_OFFSET}px)` }
                            : undefined
                        }
                      >
          ...
          <div>
            <p className="mb-1.5 text-xs font-medium text-gray-500">未配置</p>
            <SeatContainer id="unassigned" items={unassignedMembers} partColorMap={partColorMap} />
          </div>
        </div>
      </div>
      <DragOverlay>
        {(() => {
          ...
          const item = [...Object.values(placed).flat(), ...unassignedMembers].find(
            (i) => i.key === activeId,
          );
          if (!item) return null;
          ...
          return (
            <span className={`relative h-11 w-11 rounded-full border-2 ${bg} ${border} shadow-lg`}>
      ...
      {nameEdit && (
        <>
          ...
          <div
            className="fixed z-50 w-56 rounded-xl border border-gray-200 bg-white p-3 shadow-lg"
          ...
          >
            ...
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter") saveNameEdit(nameEdit.value);
            ...
            <button
              ...
              className="bg-brand-600 hover:bg-brand-700 w-full rounded-lg py-1.5 text-xs text-white"
            >
              保存
            </button>
          </div>

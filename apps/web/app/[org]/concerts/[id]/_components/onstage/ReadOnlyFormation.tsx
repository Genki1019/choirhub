...
export function ReadOnlyFormation({
  ...
  const usedCols = rowKeys.flatMap((k) =>
    containers[k].map((i) => i.col).filter((c): c is number => c != null),
  );
          ...
          <div className="space-y-2 overflow-x-auto pb-1">
            {conductor.length > 0 && (
              <div className="flex items-center gap-2">
                <span className="w-10 shrink-0 truncate text-xs text-gray-600">指揮</span>
            ...
            {piano.length > 0 && (
              <div className="flex items-center gap-2">
                <span className="w-10 shrink-0 truncate text-xs text-gray-600">ピアノ</span>
                <div className="flex flex-1 justify-center">
                  <div
                    className="flex gap-2"
                    style={
                      pattern.pianoPosition === "kamite"
                        ? { transform: `translateX(${stageContentWidth / 4}px)` }
                        : undefined
                    }
                  >
                    {piano.map((item) => (
              ...
              return (
                <div key={key} className="flex items-center gap-2">
                  ...
                  <div className="flex flex-1 justify-center">
                    ...
                    >
                      {Array.from({ length: visibleColumnCount }, (_, i) => minCol + i).map(
                        (col) => {
                          ...
                          return item ? (
                            <ReadOnlyChip
                              ...
                              item={item}
                              colorClass={
                                item.partName ? partColorMap.get(item.partName) : undefined
                              }
                            />
                          ) : (
                            <div
                              key={col}
                              className="h-11 w-11 flex-none rounded-full border border-dashed border-gray-300"
                            />
                          );
                        },
                      )}
        ...
        <div className="space-y-2 rounded-xl border border-gray-200 bg-gray-50/50 p-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            {customBoxes.map((box) => {
              ...
              return (
                <div key={box.key} className="flex min-w-0 items-center gap-3">
                  <span className="shrink-0 truncate text-xs text-gray-400">{box.title}</span>

...
function SaveTemplateModal({ orgSlug, subject, body, onClose, onSaved }: SaveTemplateModalProps) {
  ...
  const handleSave = async () => {
    ...
    if (!subject.trim()) { setError("件名が空です"); return; }
    if (!body.trim())    { setError("本文が空です"); return; }
    setSaving(true);
    setError(null);
    try {
      const template = await mailingApi.templates.save(orgSlug, { name: name.trim(), subject, body });
      onSaved(template);
      onClose();
    } catch {
      setError("保存に失敗しました");
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-xl shadow-xl w-full max-w-sm p-6 space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="font-semibold text-gray-800 text-sm">テンプレートとして保存</h3>
          <button aria-label="閉じる" onClick={onClose} className="text-gray-400 hover:text-gray-600"><X size={16} /></button>
        </div>
        <div>
          <label className="block text-xs font-medium text-gray-600 mb-1.5">テンプレート名 <span className="text-red-500">*</span></label>
          <input
...
function TemplatePanel({ orgSlug, onApply, onClose }: TemplatePanelProps) {
  ...
  useEffect(() => {
    mailingApi.templates.list(orgSlug)
  ...
  return (
    <div className="border-t border-gray-100 bg-gray-50">
      <div className="flex items-center justify-between px-4 py-2 border-b border-gray-100">
        ...
        <button aria-label="テンプレートパネルを閉じる" onClick={onClose} className="text-gray-400 hover:text-gray-600"><X size={14} /></button>
      </div>
      <div className="max-h-44 overflow-y-auto">
        {loading && (
          <div className="flex items-center gap-2 text-gray-400 px-4 py-3">
            <Loader2 size={13} className="animate-spin" /><span className="text-xs">読み込み中...</span>
          </div>
        )}
        {!loading && templates.length === 0 && (
          <p className="text-xs text-gray-400 px-4 py-3">保存済みテンプレートがありません</p>
        )}
        {templates.map((t) => (
            ...
            {confirmDeleteId === t.id ? (
              <div className="flex items-center justify-between px-4 py-2.5 bg-red-50">
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    {deleting === t.id ? <Loader2 size={11} className="animate-spin inline" /> : "削除"}
                  </button>
                  <button
              ...
              <div className="flex items-center justify-between px-4 py-2.5 hover:bg-white group">
                <button
                ...
                <button
                  ...
                  disabled={deleting === t.id}
                  className="ml-2 text-gray-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity disabled:opacity-50 shrink-0"
...
export function ComposeModal({ orgSlug, parts, onClose, onSent }: ComposeModalProps) {
  ...
  const handleSend = async () => {
    ...
    if (recipientType === "part"   && selectedPartIds.size === 0)   { setError("パートを選択してください"); return; }
    if (recipientType === "role"   && selectedRoles.size === 0)     { setError("ロールを選択してください"); return; }
    if (recipientType === "custom" && selectedMemberIds.size === 0) { setError("送信先を選択してください"); return; }

    setSending(true);
    setError(null);
    try {
      await mailingApi.send(orgSlug, { subject: subject.trim(), body: body.trim(), recipientType, recipientFilter: buildFilter() });
      onSent();
      onClose();
    } catch (err: unknown) {
  ...
  return (
    <>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] flex flex-col">
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 shrink-0">
            <h2 className="font-semibold text-gray-800 text-sm">新規メール</h2>
          ...
          <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1.5">件名 <span className="text-red-500">*</span></label>
              <input
            ...
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1.5">送信先</label>
          ...
          {showTemplatePanel && (
            <TemplatePanel
              orgSlug={orgSlug}
              onApply={(t) => { setSubject(t.subject); setBody(t.body); }}
      ...
      {showSaveModal && (
        <SaveTemplateModal
          orgSlug={orgSlug} subject={subject} body={body}

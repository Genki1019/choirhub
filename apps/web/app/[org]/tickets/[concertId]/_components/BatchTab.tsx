...
export function BatchTab({
  ...
  batch.allocations.forEach((row) => {
    ...
    if (!partMap.has(key)) {
      partMap.set(key, {
        partName: row.partName ?? "パート未設定",
        sortOrder: row.partSortOrder,
        voiceType: row.partVoiceType,
        rows: [],
      });
    }
  ...
  const totalSold = batch.allocations.reduce(
    (s, a) => s + a.soldAdult + a.soldStudent + a.soldOther,
    0,
  );
    ...
    <div className="space-y-4">
      ...
      <div className="overflow-hidden rounded-xl border border-gray-200 bg-white">
        ...
        <div className="overflow-x-auto">
          <div className="flex min-w-max items-center gap-3 border-b border-gray-100 bg-gray-50 px-4 py-2.5 text-xs font-medium text-gray-500">
            <span className="w-28 shrink-0">名前</span>
            <span className="w-16 shrink-0">配布</span>
            <span className="w-12 shrink-0 text-center">大人</span>
            <span className="w-12 shrink-0 text-center">学生</span>
            <span className="w-12 shrink-0 text-center">他</span>
            <span className="w-12 shrink-0 text-center">返却</span>
            <span className="w-10 shrink-0 text-center">残</span>
            <span className="w-10 shrink-0 text-center">集金</span>
          </div>

          {sortedParts.map(({ partName, rows }) => (
            <div key={partName}>
              <div className="border-b border-gray-100 bg-gray-50/70 px-4 py-1.5">

...
export default function Sidebar({
  ...
  const isVisitor =
    ...
    !roles.some((r) =>
      ["admin", "tech", "score", "member", "ticket", "finance", "conductor"].includes(r),
    );
    ...
    <>
      ...
      <aside
        className={[
          ...
          isOpen ? "translate-x-0 lg:w-52" : "-translate-x-full lg:w-0 lg:overflow-hidden",
        ].join(" ")}
      >
        <button
          className="absolute top-3 right-3 rounded-md p-1 text-gray-400 hover:bg-gray-100 lg:hidden"
          onClick={onClose}
          aria-label="メニューを閉じる"
        >
          <X size={18} />
        ...
        <div className="border-b border-gray-100 px-5 pt-5 pb-3">
          <p className="text-brand-600 text-lg leading-tight font-bold">ChoirHub</p>
          <Link
            href="/select-org"
            prefetch={false}
            className="mt-1.5 flex w-full items-center justify-between rounded-md px-2 py-1 text-xs text-gray-500 transition-colors hover:bg-gray-100"
            title="団体を切り替える"
          >
            <span className="truncate">{orgName}</span>
            <ChevronsUpDown size={12} className="ml-1 shrink-0 text-gray-400" />
          </Link>
        </div>
        ...
        <nav className="flex-1 py-3">
          ...
          {showSettings && (
            <>
              ...
              {settingsOpen && (
                <div className="pb-1">
                  {(isAdmin ? SETTINGS_NAV_ITEMS : SETTINGS_FINANCE_ITEMS).map(
                    ({ label, suffix }) => {
                      ...
                      return (
                        <Link
                          ...
                          prefetch={false}
                          className={[
                            "flex items-center border-l-2 py-2 pr-5 pl-12 text-sm transition-colors",
                            isActive
                              ? "border-brand-500 bg-brand-50 text-brand-600 font-medium"

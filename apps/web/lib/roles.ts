...
export const ROLE_BADGE_STYLES: Record<string, { label: string; className: string }> = Object.fromEntries(
  ROLES.map((r) => [r.key, { label: r.defaultName, className: r.badgeClass }])
);

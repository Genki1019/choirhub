...
export const settingsApi = {
  ...
  deletePart: (orgSlug: string, partId: string) =>
    apiClient.delete(`/${orgSlug}/parts/${partId}`),

  getFee: (orgSlug: string) =>
    apiClient.get<FeeSettings>(`/${orgSlug}/settings/org`),

  updateFee: (orgSlug: string, data: { feeType?: "per_rehearsal" | "monthly"; defaultFeeAmount?: number | null }) =>
  ...
  updateExpenseCategory: (orgSlug: string, categoryId: string, data: { name?: string; sortOrder?: number }) =>
  ...
  createMemberType: (orgSlug: string, data: { name: string; defaultFeeAmount?: number | null; sortOrder?: number }) =>
    apiClient.post<MemberType>(`/${orgSlug}/settings/member-types`, data),

  updateMemberType: (orgSlug: string, typeId: string, data: { name?: string; defaultFeeAmount?: number | null; sortOrder?: number }) =>
  ...
  createEventCategory: (orgSlug: string, data: { name: string; color?: string; sortOrder?: number }) =>
    apiClient.post<EventCategory>(`/${orgSlug}/settings/event-categories`, data),

  updateEventCategory: (orgSlug: string, categoryId: string, data: { name?: string; color?: string; sortOrder?: number }) =>

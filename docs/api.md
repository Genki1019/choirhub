...
| [支出カテゴリ追加](#expense-categories-create)   | POST   | `/:orgSlug/settings/expense-categories`     | finance+ |
| [支出カテゴリ更新](#expense-categories-patch)    | PATCH  | `/:orgSlug/settings/expense-categories/:id` | finance+ |
| [支出カテゴリ削除](#expense-categories-delete)   | DELETE | `/:orgSlug/settings/expense-categories/:id` | finance+ |
...
**権限**: 認証済み全員（ロールによる絞り込みチェックはない）

> `guest`/`visitor` ロールを持つメンバー自身は、閲覧者のロールに関わらず一覧結果から常に除外される（`EXCLUDE_HIDDEN_ROLES`）。
  ...
  "data": [
    {
      ...
      "memberType": { "id": "cuid", "name": "正団員", "defaultFeeAmount": null },
      "roles": ["member"],
      "status": "active",
      "bio": "よろしくお願いします",
      "job": "エンジニア",
...
| フィールド  | 型     | 必須 | 説明                                                                                              |
| ----------- | ------ | ---- | ------------------------------------------------------------------------------------------------- |
| label       | string | ✓    | 種別ラベル（フライヤー/しおり/行程表/資料/その他 等。練習録音の場合は曲名・パート名等の自由入力） |
...
> 許可される拡張子は `.pdf` / `.jpg` / `.jpeg` / `.png` / `.mp3` / `.wav` のみ（`.mp3`/`.wav`は練習録音アーカイブ用途）。
...
| パラメータ | 型     | 説明                                                                                       |
| ---------- | ------ | ------------------------------------------------------------------------------------------ |
| `q`        | string | タイトル・作曲者・編曲者を対象にした部分一致検索（大文字小文字区別なし）。省略時は全件返却 |
...
| ---------------------------------------- | --------------------------------------- | --------------------------------------- | --------------------------------------- |
| `visitor`                                | PDF のみインライン表示、MIDI/音声は 403 | PDF のみインライン表示、MIDI/音声は 403 | PDF のみインライン表示、MIDI/音声は 403 |
| `member`（購入済み）                     | 403                                     | OK                                      | OK                                      |
| `score` / `tech` / `conductor` / `admin` | OK                                      | OK                                      | OK                                      |
...
> 許可される拡張子は `.pdf` / `.jpg` / `.jpeg` / `.png` / `.mp3` / `.wav` のみ（イベントの添付ファイルAPIと共通実装のため、演奏会側でも音声形式が拒否されない）。
> レスポンスの`contentType`はサーバーが拡張子から一意に決定した値（クライアント指定の`contentType`は無視する。偽装したContentTypeでR2に保存されるのを防ぐため）。実際のPUTリクエストはこの値と`Content-Type`ヘッダーを一致させる必要がある（不一致だと署名検証で拒否される）。
...
**権限**: 公開済み（`racePublishedAt`あり）なら認証済み全メンバー / 未公開は`ticket or admin`のみ
...
**権限**: 認証済み全員（ロールによる絞り込みチェックはない。自身が申請/参加したもののみ返却。admin/ticket は全件返却）
...
見学申込を承認する。Member/Userは作成せず、ステータスのみ更新する。団体の紹介文テンプレート（[GET/PATCH `/settings/visitor-intro-template`](#settings-visitor-intro-template-get)）を使って組み立てた団員向けの紹介メール下書き（`draft`）を返す。
...
<a id="settings-visitor-intro-template-get"></a>

### GET `/api/v1/:orgSlug/settings/visitor-intro-template`

見学者紹介文（承認後の紹介メール・コピー用テキスト）のテンプレートを取得する。未設定の団体にはデフォルトのテンプレートが返る。
...
<a id="settings-visitor-intro-template-patch"></a>

### PATCH `/api/v1/:orgSlug/settings/visitor-intro-template`

見学者紹介文のテンプレートを更新する。`bodyTemplate` 内の `{lines}` に見学者ごとの行（`lineTemplate` を1名ずつ展開したもの）が挿入される。`lineTemplate` では `{name}`（氏名）・`{part}`（希望パート）・`{origin}`（出身団体）が使える。`[...]` で囲んだ区間は、中で参照している変数がすべて空の場合その区間ごと非表示になる（例: `[ / 出身団体: {origin}]` は出身団体が未入力なら丸ごと消える）。`[...]` の外で使われている変数が空の場合は「未定」に置き換わる。

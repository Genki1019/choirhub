...
見学者紹介文のテンプレートを更新する。`bodyTemplate` 内の `{lines}` に見学者ごとの行（`lineTemplate` を1名ずつ展開したもの）が挿入される。`lineTemplate` では `{name}`（氏名）・`{part}`（希望パート）・`{origin}`（出身団体）が使える。`{part}`/`{origin}` は未入力の見学者には「未定」に置き換わる。

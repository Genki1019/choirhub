...
| メールアドレス変更確認   | `/email-change/[token]`   | 公開           |
| 団体選択                 | `/select-org`             | ログイン済み   |
| 団体作成申請             | `/apply`                  | 公開           |
| システム管理者コンソール | `/admin`                  | システム管理者 |
| お問い合わせ             | `/contact`                | 公開           |
| プライバシーポリシー     | `/privacy`                | 公開           |
| 利用規約                 | `/terms`                  | 公開           |

> 「公開」「未認証のみ」の画面は未ログインでも開ける。それ以外の画面は、セッションCookieが無ければ`proxy.ts`が`/login`へリダイレクトする。公開パスは完全一致か`/`区切りの配下のみを対象とし（`/contact-choir`のような団体スラグは公開扱いにしない）、公開画面を追加する際は`proxy.ts`の`PUBLIC_PATHS`にも追加する
...
| 資料ライブラリ                   | `/[org]/library`                               | guest+（サイドバーでは visitor に非表示。「資料」タブは guest には accessLevel: public のみ表示。楽譜/本番/イベントタブの閲覧可否はタブごとに各画面と同じ判定） |
...
| 通知一覧                         | `/[org]/notifications`                         | visitor+（自分宛の通知のみ）                                                                                                                                    |
...
#### 通知ベル（ヘッダー右上、`UserMenu`の左、`NotificationBell`）

- ベルアイコンをクリックしてドロップダウンを開閉（60秒間隔でポーリング）
- 未読件数がある場合はベル右上に赤い丸バッジを表示（10件以上は `9+`）
- ドロップダウン: 直近8件を表示（未読は太字＋左端に青いドット＋薄い青背景、既読は通常表示）。「すべて既読にする」ボタン（未読がある場合のみ表示）、末尾に「すべて見る」リンク（`/[org]/notifications`へ）
- 通知クリックで既読化 → `link`先へ遷移してメニューを閉じる
- メニュー外クリックで自動的に閉じる
...
- ChoirHubロゴ・「プライバシーポリシー」（`/privacy`）「利用規約」（`/terms`）「お問い合わせ」（`/contact`）・コピーライト表記
- トップページ（3.1）と[3.3d〜3.3e](#33d-お問い合わせ-contact)の各ページのフッターも同じリンク構成（リンク定義は`lib/legal-links.ts`で共通化）
...
**目的**: 未ログインユーザーへの紹介と、ログイン・団体作成申請への誘導

#### レイアウト

- ヘッダー（スクロール時も固定）: ロゴ + 「団体作成を申請」ボタン（`/apply`へ）+ ログインリンク
- ヒーロー: キャッチコピー + 「ログインしてはじめる」ボタン + 「団体作成を申請する」ボタン（`/apply`へ）
- フィーチャー: 主要機能の紹介（3〜4項目）
- CTA: 「ログイン」ボタン + 「新しく団体を立ち上げたい方は団体作成を申請する」リンク（`/apply` へ）
- フッター: ロゴ + 「プライバシーポリシー」「利用規約」「お問い合わせ」リンク + コピーライト
...
「団体作成を申請する」を開くと申請フォーム（[3.3a 団体作成申請フォーム](#33a-団体作成申請フォーム共有コンポーネント) 参照）が展開される。氏名・メールアドレスはログインユーザーの情報でプリフィルされる。
...
- 画面表示時に自身の所属団体一覧とシステム管理者フラグ（`isSystemAdmin`）を取得。未ログイン（401） → `/login` へ自動リダイレクト。それ以外のエラー（500等） → 「団体情報の取得に失敗しました。しばらくしてから再度お試しください。」を表示（0件時の空状態とは別メッセージ）
...
- 「団体作成を申請する」→ インラインフォームを展開（団体作成申請フォーム、氏名・メールはプリフィル）
  - `[×]` ボタンでフォームを閉じる
- システム管理者（`isSystemAdmin`）の場合のみ「システム管理者コンソール」リンクを表示 → クリックで `/admin` へ遷移
- 「ログアウト」→ ログアウトAPIを呼び出し `/login` へ遷移

---

### 3.3a 団体作成申請フォーム（共有コンポーネント）

**目的**: `/select-org`（3.3）・`/apply`（3.3b）の団体作成申請と、`/admin`（3.3c）の団体作成の3箇所から使う共有フォーム。送信先の関数（`submitFn`）と成功メッセージ・送信ボタンのラベルを差し替えて再利用する。既定は申請用（`POST /auth/org-applications`、認証不要）。

#### フィールド

- 団体名（必須）
- スラグ（必須。URLに使用。団体名から自動生成されるが手動編集も可能。一度編集すると自動生成には戻らない。申請の場合は承認時にシステム管理者が確認・変更できる）
- パート構成（`<select>`、選択肢順: 混声四部→女声三部→男声四部→カスタム（あとで手動設定））
- 管理者氏名（必須。`/select-org` から開いた場合はログインユーザーの氏名でプリフィル）
- 管理者メールアドレス（必須。`/select-org` から開いた場合はログインユーザーのメールアドレスでプリフィル。作成・承認されるとこのアドレス宛に招待メールが届く）
- メッセージ（任意）
- 送信ボタンの下に「送信することで、利用規約とプライバシーポリシーに同意したものとみなします。」（各リンクは別タブで開く）

#### インタラクション

- 未入力・不正な値のまま送信すると項目ごとにバリデーションエラーを表示（送信ボタンは送信処理中のみ無効化。他の React Hook Form 採用フォームと同じ作法）
- 送信成功 → フォームの代わりに成功メッセージを表示（申請用途: 「送信しました。システム管理者の承認をお待ちください。」／作成用途: 「団体を作成し、招待メールを送信しました。」）
- エラー（409：スラグ重複） → 「このスラグはすでに使用されています」（`/admin`での作成時のみ発生しうる。申請投稿自体はスラグ重複チェックを行わないため無害）
- エラー（400：スラグ形式不正・予約語） → APIのメッセージをそのまま表示（例: 「このスラグはシステムで使用しているため使えません」。`admin`・`contact`等の固定ルートと同名のスラグは使えない）
- エラー（上記以外） → 「送信に失敗しました。しばらくしてから再試行してください」

---

### 3.3b 団体作成申請 `/apply`

**目的**: ChoirHubのアカウントを持たない訪問者でも団体作成を申請できる、ログイン不要の単独ページ。トップページ（3.1）のヘッダー・ヒーロー・CTA、およびログイン画面（3.2）から遷移する。
...
- フォームの挙動は [3.3a](#33a-団体作成申請フォーム共有コンポーネント) と同一（氏名・メールのプリフィルなし）
- 「ログイン」リンク → `/login` へ遷移

---

### 3.3c システム管理者コンソール `/admin`

**目的**: システム管理者が団体作成申請を確認・承認・却下する、申請を経由せず団体を作成する、運営への問い合わせを確認する、および削除済み団体を復元する

**表示条件**: システム管理者（環境変数 `SYSTEM_ADMIN_EMAILS` 登録者）のみ。サーバーサイドで判定し、未ログインなら `/login` へ、システム管理者でなければ `/select-org` へリダイレクトする
...
│  │ スラグ: tokyo-men-choir         │  │
│  │ 削除日: 2026/9/24（admin@...）  │  │
│  │ 完全削除予定: 2026/10/24（残り30日）│ │
│  │                         [↺ 復元]│  │
│  └────────────────────────────────┘  │
└──────────────────────────────────────┘
```

「＋ 団体を作成する」を開くと[3.3a 団体作成申請フォーム](#33a-団体作成申請フォーム共有コンポーネント)と同じフィールド構成のフォームが展開される（送信ボタンは「作成する」）。

保留中の申請が0件の場合は「保留中の申請はありません」、未対応のお問い合わせが0件の場合は「未対応のお問い合わせはありません」、削除済みの団体が0件の場合は「削除済みの団体はありません」を表示する。

#### インタラクション

- 画面表示時に保留中（`status=pending`）の申請一覧を取得。取得失敗時はエラーメッセージを表示
- 「＋ 団体を作成する」→ フォームを展開。送信すると `POST /auth/orgs` を呼び、成功すると`OrgApplication`を作らずに団体・パート・招待メールが即座に作成/送信され「団体を作成し、招待メールを送信しました。」を表示。スラグが既存団体と重複している場合は「このスラグはすでに使用されています」を表示（[3.3a](#33a-団体作成申請フォーム共有コンポーネント)の409ハンドリングと共通）。`[×]` ボタンでフォームを閉じる
- 各申請カードの「スラグ」欄は申請時の値で初期表示され、承認前に編集できる（システム管理者による確認・修正）。値が形式（英小文字・数字・ハイフン、2〜50文字）を満たさない間は「承認」ボタンを無効化する
- 「承認」→ 編集後（未編集ならそのまま）のスラグを添えて `POST /auth/org-applications/:id/approve` を呼ぶ。成功すると団体・パート・招待メールが作成/送信され、一覧から該当行が消える。スラグが既存団体と重複している場合は409が返り「このスラグはすでに使用されています」を表示。スラグ形式が不正（通常はボタンの無効化で防がれる）または予約語（`admin`・`contact`等の固定ルート名）の場合は400が返り、APIのメッセージ（例: 「このスラグはシステムで使用しているため使えません」）を表示（いずれも一覧には残るので再編集して再試行できる）
- 「却下」→ `POST /auth/org-applications/:id/reject` を呼ぶ。成功すると一覧から該当行が消える
- 上記以外の操作失敗時は「操作に失敗しました。もう一度お試しください。」を表示
- 「未対応のお問い合わせ」は `GET /auth/inquiries?status=open` で取得し、新しい順に種類・受付日・送信者（氏名・メールアドレス）・団体名（入力時のみ）・本文（改行を保持）を表示する。取得失敗時は「お問い合わせの取得に失敗しました。しばらくしてから再度お試しください。」を表示
- 「対応済み」→ `POST /auth/inquiries/:id/resolve` を呼ぶ。成功すると一覧から該当行が消える。失敗時は「更新に失敗しました。もう一度お試しください。」を表示し一覧に残す。返信は送信者のメールアドレス宛に個別に行う（画面内に返信機能は無い）
- 「削除済みの団体」は `GET /auth/orgs/deleted` で取得し、削除日の新しい順に表示する。取得失敗時は「削除済み団体の取得に失敗しました。しばらくしてから再度お試しください。」を表示
- 「復元」→ `POST /auth/orgs/:id/restore` を呼ぶ。成功すると一覧から該当行が消え、団員は再びその団体を利用できる（復元の通知メールは送らない）。失敗時は「復元に失敗しました。もう一度お試しください。」を表示し一覧に残す
- 今後システム管理者向けの機能（団体一覧の横断参照など）を追加していく置き場として、`/admin` 配下は独立したレイアウトにしている（団体管理者・団員向けの `[org]` 配下とは分離）

---

### 3.3d お問い合わせ `/contact`

**目的**: ChoirHub運営（システム管理者）への問い合わせ窓口。団体の復元依頼・個人情報の開示等の請求などを受け付ける。未ログインでも利用できる

#### レイアウト

```text
┌──────────────────────────────────────┐
│  ChoirHub ロゴ                        │ ← ヘッダー（トップへのリンク）
├──────────────────────────────────────┤
│  お問い合わせ                          │
│  ChoirHub運営へのお問い合わせは...      │
│  団体内の運用に関するご質問は所属団体の  │
...
> 種類の選択肢: 削除した団体の復元 / アカウント・ログイン / 個人情報の開示・訂正・削除 / 不具合の報告 / その他。3.3d〜3.3eは共通レイアウト（`app/(info)/layout.tsx`: ロゴヘッダー + 本文 + `AppFooter`）を使う

#### インタラクション

- `?category=org_restore` 等のクエリで種類を初期選択する（削除確認モーダル・利用規約・プライバシーポリシーからの導線）。不正な値は無視して「その他」にする
- お名前・メールアドレス・お問い合わせ内容が未入力／不正、または団体名・内容が文字数上限（100文字・2000文字）を超えたまま送信すると、項目ごとにバリデーションエラーを表示
- 送信 → `POST /auth/inquiries`。成功するとフォームの代わりに「お問い合わせを受け付けました。内容を確認のうえ、ご入力のメールアドレスへご連絡します。」を表示
- レート制限（429）→ APIのメッセージ（「しばらく時間をおいてから再試行してください」）を表示。その他の失敗 → 「送信に失敗しました。しばらくしてから再試行してください」

---

### 3.3e プライバシーポリシー `/privacy`・利用規約 `/terms`

**目的**: 個人情報の取扱いとサービスの利用条件を示す静的ページ。未ログインでも閲覧できる
...
> 条項の見出し・本文のスタイルは共通コンポーネント`LegalDocument`・`LegalSection`で統一する。デモ環境への個人情報入力の注意（利用規約「第2条」直後・プライバシーポリシー「3. 団体内での共有」内）は`LegalWarning`で赤字の注意書きとして強調する

#### インタラクション

- プライバシーポリシーの「開示・訂正・利用停止等の請求」内のリンク → `/contact?category=privacy`
- 利用規約「第6条（団体の削除とデータの取扱い）」内のリンク → `/contact?category=org_restore`
- 各ページ内の相互リンク（利用規約 → プライバシーポリシー等）

---

### 3.4 招待受諾 `/invite/[token]`

**目的**: 招待リンクを通じたメンバー登録。招待先メールアドレスが既存の`User`かどうか（`isExistingUser`）でフォームの内容が変わる
...
招待先メールアドレスが既に`User`として登録済みの場合（別団体の管理者が新団体の管理者として招待された場合など）は、新規アカウント作成ではなく既存アカウントへの`Member`追加として扱う。お名前・パスワード確認欄は表示せず、現在のパスワードで本人確認を行う。
...
新規ユーザーは登録完了後、同じカード内が以下の完了画面に切り替わる。
...
既存ユーザーはパスワード検証の時点で本人確認済みのため、完了画面を経由せずセッションが発行され、そのまま新団体の画面（`/{orgSlug}`）へ自動遷移する。
...
- 新規ユーザー: お名前未入力・パスワード8文字未満・パスワード（確認）不一致はクライアント側で即時バリデーションエラーを表示
- 既存ユーザー: パスワード未入力はクライアント側で即時バリデーションエラーを表示
- 新規ユーザーの登録成功 → カード内が完了画面に切り替わる。「ログインページへ」ボタンをクリックすると `/login` へ遷移（自動リダイレクトはしない）
- 既存ユーザーの登録成功 → `POST /auth/invite/:token` のレスポンスに含まれる `orgSlug` へ自動遷移（`/{orgSlug}`）。完了画面は表示しない
- エラー（409：登録済み） → 「このメールアドレスはすでに登録済みです。ログインページからログインしてください。」
- エラー（401：既存ユーザーのパスワード不一致） → 「パスワードが正しくありません」
- エラー（409・401以外） → 「登録に失敗しました。もう一度お試しください。」
...
| 今月の幹事                | 表示専用（編集不可）。当月を含む幹事期間が設定されているパート名を自動表示。未設定時は「未設定」をグレー表示。設定は「10.5 パートレース」画面で行う |
...
- `admin`（他者閲覧時）: ヘッダーに「管理者操作」ボタン表示 → [5.3](#53-メンバー管理者操作-orgmembersidmanageadmin)へ遷移

---

### 5.3 メンバー管理者操作 `/[org]/members/[id]/manage`（admin）

**目的**: ロール・パート・ステータス・管理者メモの変更、退団処理を行う管理者専用画面。一般団員向けのプロフィール閲覧（5.2）とは別画面に分離し、一般団員には表示しない
...
> 誰の情報を編集しているか分かるよう、メンバー詳細（5.2）と同じプロフィールカードを画面上部に表示する（編集不可・表示のみ）。
> `admin` 以外が直接アクセスした場合は「このページにアクセスする権限がありません」を表示する。

#### インタラクション

- 「ロール」ラベル横の「?」アイコンをホバー/クリック → 全ロール（`member`は常に付与されるため選択肢に含まれない）の権限説明を一覧ポップアップ表示（外側クリック・Escapeで閉じる）
- ロール・パート・ステータス・メンバー区分・管理者メモを変更し「変更を保存」→ メンバー詳細画面（5.2）へ遷移
- メールアドレスは確認リンクなしで即時変更できる（本人による[5.4a](#54a-メールアドレス変更確認リンク方式)の確認リンク方式とは異なる経路）。変更すると対象メンバーの既存ログインは全て無効化され、新・旧両方のアドレスに変更完了メールが送られる。既に使用中のアドレスを指定した場合はエラーメッセージを表示する
- 「退団処理」→ 確認ダイアログ→OKでソフトデリート（`deletedAt`セット）→ メンバー一覧へ遷移

---

### 5.4 プロフィール編集 `/[org]/members/[id]`（インライン）
...
> メールアドレスの変更はこのフォームでは行わない。[5.4a メールアドレス変更](#54a-メールアドレス変更確認リンク方式)を参照。
...
### 5.4a メールアドレス変更（確認リンク方式）

**目的**: ログイン識別子であるメールアドレスの変更を、新アドレス所有確認を挟んだ非同期フローで行う（即時反映は行わない）

#### 申請UI（メンバー詳細 `/[org]/members/[id]`、本人閲覧時にプロフィール情報の下へ常時表示）
...
- 送信 → 新アドレス宛に確認メールを送信し、フォームを「〇〇宛に確認メールを送信しました」というインライン成功表示に切り替える
- 対象アドレスが既に他ユーザーに使用されている場合も、正常系と同一のレスポンス・表示になる（列挙攻撃防止のため区別しない）

#### 確認ページ `/email-change/:token`（公開・ログイン不要）
...
- ページ表示時にトークンを検証し、変更先メールアドレスを取得（リンク無効・期限切れ時はエラーメッセージのみ表示）
- 「このメールアドレスに変更する」→ 変更確定・完了画面に切り替え（「ログインページへ」リンク）。確定後は安全のため対象ユーザーの全セッションが無効化される
- 既に他ユーザーが同じアドレスを取得済みの場合はエラーメッセージを表示する

---

### 5.5 見学申込 `/[org]/members/applications`（admin）
...
- 「外部カレンダーに連携」ボタン（全ロール共通） → 外部カレンダー連携モーダルを開く
- 「イベント追加」ボタン（`admin`/`tech`/`conductor`） → `/[org]/schedule/new`（専用ページ）

**外部カレンダー連携モーダル**（全ロール共通）

- 未発行時は「発行する」ボタンのみ表示。押下でトークンを発行し、フィードURL・「Googleカレンダーに追加」・「URLをコピー」を表示する
- 「Googleカレンダーに追加」ボタン → `https://calendar.google.com/calendar/r/settings/addbyurl?url={フィードURL}` を新規タブで開く
- 「URLをコピー」ボタン → フィードURL（`/api/v1/calendar/{orgSlug}/feed.ics?token={token}`）をクリップボードにコピー
- 「再発行する」ボタン → トークンを再発行し、旧URLは無効化される（再発行後は新しいURLに切り替わる旨を注記表示）
...
├─────────────────────────────────────────────────┤
│  📎 練習録音（練習カテゴリのイベントのみ表示）      │
│  [第4楽章] movement4.mp3  ▶───────  [追加][削除] │
├──────────────────┬────────┬────────────────────┤
│  名前             │  出欠  │  メモ               │
├──────────────────┼────────┼────────────────────┤
│  Tenor I  ○4 △1 ✕1 ──────────────────────────  │
...
> 練習録音セクションはカテゴリが「練習」（`slug: "rehearsal"` またはカテゴリ名が「練習」）のイベントのみ表示する。添付ファイルと同じ`EventFile`を利用するが、拡張子（`.mp3`/`.wav`）で振り分けて別セクションに表示し、ファイル行は`<audio>`プレーヤーで再生できる。ラベルはプリセット選択ではなく曲名・パート名等の自由入力。
...
- `admin`/`tech`/`conductor`/`score` のみ（練習カテゴリのイベント）: 練習録音セクションでラベルを自由入力（曲名・パート名等）+ 音声ファイル（.mp3/.wav）選択 → 「追加」クリックでアップロード。削除操作は添付ファイルと同様
...
（⠿ = ドラッグハンドル。ステージ・曲目行ともドラッグ&ドロップでのみ並び替え可能）

#### インタラクション（admin のみ）

- ✏️ アイコン（ステージ名横）→ インラインテキスト入力に切り替え → Enter/blur で保存
- ステージの ⠿ ドラッグ&ドロップ → ステージの表示順を入れ替え。`PUT /concerts/:id/stages/order`に並び替え後の全ステージID配列を一括送信（失敗時は元の順序に戻す）
- 演目行の ⠿ ドラッグ&ドロップ → 同一ステージ内で演目の表示順を入れ替え。`PUT /concerts/:id/stages/:stageId/programs/order`に並び替え後の全演目ID配列を一括送信（失敗時は元の順序に戻す）
- 演目行 ↔ ボタン → 演目移動/コピーモーダルを開く
- 「曲目を追加」→ `/[org]/concerts/[id]/programs/new?stageId={ステージID}`（専用ページ）
- 「ステージを追加」→ ステージ追加モーダルを開く

**曲目追加ページ** `/[org]/concerts/[id]/programs/new`（`admin` のみ）

新しく作成／既存から選ぶの2タブ構成。「既存から選ぶ」の検索はタイトル・作曲者・編曲者を対象にした部分一致検索（サーバー側、300msデバウンス）で、演奏会未定・登録済み問わず全楽譜が対象。

| モード       | フィールド | 入力形式            | 必須 | 備考                                 |
| ------------ | ---------- | ------------------- | ---- | ------------------------------------ |
| 新しく作成   | 曲名       | テキスト            | ✅   | autoFocus                            |
| 新しく作成   | 作曲者     | テキスト            |      |                                      |
| 新しく作成   | 編曲者     | テキスト            |      |                                      |
| 既存から選ぶ | 検索       | テキスト            |      | 空欄の場合は全楽譜をタイトル順で表示 |
| 既存から選ぶ | 楽譜選択   | 検索結果から1件選択 | ✅   |                                      |

- 「追加する」→ 登録後、本番詳細のステージ構成タブ（`?tab=stages`）に戻る
- 「キャンセル」→ 保存せずステージ構成タブに戻る
...
> 見学申込（[5.5](#55-見学申込-orgmembersapplicationsadmin)）の承認画面から「今すぐ紹介メールを送る」で開いた場合は、件名・本文・送信先（全員）が事前入力された状態で開く。
...
| 要素                              | 表示条件                                                                                                                                                             |
| --------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 「採点設定」ボタン                | `isTicketManager`のみ表示。`racePublishedAt`設定済み（公開後）は非活性                                                                                               |
| 「全体に公開」/「公開取消」ボタン | `isTicketManager`のみ                                                                                                                                                |
| 公開済みバナー                    | `racePublishedAt`が設定されている場合                                                                                                                                |
| パート順位タブ                    | パート別の合計ポイント順位・内訳チップ・統計値・幹事期間を表示                                                                                                       |
| 個人順位タブ                      | メンバーごとの順位・販売枚数・達成率・情宣回数を表示                                                                                                                 |
| ポイントルール（`ScoringRules`）  | 常時表示（折りたたみ）。基準は`enabled`な項目のみ表示（無効化された基準は非表示）                                                                                    |
| 各パートカードの幹事期間表示      | `isTicketManager`は鉛筆アイコンで編集可（未設定でも表示）。それ以外は設定済みの場合のみ表示専用で表示、未設定なら非表示。`racePublishedAt`の状態に関わらず常に編集可 |

#### インタラクション

- タブ切り替え → パート順位/個人順位を切り替え表示（グラフ描画は無し、数値・チップのみ）
- 「採点設定」→ 編集モーダル（`ScoringSettingsModal`）表示 → 各基準の有効/無効・配点・（speed系のみ）枚数閾値/最低人数を編集 → 保存 → PATCH `/tickets/:concertId/scoring` → 一覧を再取得し即時反映
- 「全体に公開」→ POST `/tickets/:concertId/race/publish` → 全団員が結果を閲覧可能に（`my`ページにバナー表示）
- 「公開取消」→ DELETE `/tickets/:concertId/race/publish`
- パートカードの「幹事期間」の鉛筆アイコンをクリック → その場で開始月・終了月（`<input type="month">`）を編集 → チェックで保存 → PATCH `/tickets/:concertId/race/organizer-periods/:partId` → 該当パートのみ即時反映（一覧再取得なし）。両方空欄で保存すると設定解除
...
| 「危険な操作」セクション | `admin`のみ表示                                                                      |

#### インタラクション

- 団体名を変更 → 「保存する」（`admin`のみ）→ PATCH `/settings` → 「保存しました」を2.5秒間表示。保存後の団体名は削除確認モーダルの確認用団体名にも即時反映される
- 「削除」（`admin`のみ）→ 団体削除確認モーダルを開く
...
│  ・団員（体験を除く）に通知メール   │
│  ・30日後にすべてのデータを完全削除 │
│  ・30日以内なら運営への問い合わせで復元可│
│                                    │
│ 確認のため団体名 東京男声合唱団 を入力│
│ [                                ] │
│ パスワード                          │
│ [                                ] │
│                                    │
...
| 状況                            | 表示・挙動                                                                                                                                                                                                                                                    |
| ------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 団体名不一致 / パスワード未入力 | 「団体を削除する」ボタンを無効化                                                                                                                                                                                                                              |
| 送信成功                        | `POST /settings/delete` 成功後、`/select-org` へ遷移（削除した団体は一覧に表示されない）                                                                                                                                                                      |
| 4xxエラー                       | APIのエラーメッセージをそのまま表示（`role="alert"`）。例: パスワード不一致（403）→「パスワードが正しくありません」、削除保護された団体（403）→「デモ環境のため、この団体は削除できません」、レート制限（429）→「しばらく時間をおいてから再試行してください」 |
| 5xxエラー・通信失敗             | 「削除に失敗しました。もう一度お試しください。」を表示                                                                                                                                                                                                        |

- 「ChoirHub運営へのお問い合わせ」リンク → `/contact?category=org_restore` を別タブで開く（入力中のモーダルを閉じずに済むように）
- 「キャンセル」または Esc キーでモーダルを閉じる（送信中は閉じない）
- パスワード欄の直前に、画面上は見えないユーザー名欄（`autocomplete="username"`）を置く。ブラウザのパスワードマネージャーが保存済みのメールアドレスを団体名欄へ自動入力しないようにするため
- `role="dialog"`・`aria-modal`・見出しへの`aria-labelledby`を付与する（共通Modal化は Issue #159 で対応予定）
...
├──────────────────────────────────┤
│  ⠿ Tenor I            [✏️][🗑️]│ ← 並び替え（ドラッグハンドル）・編集・削除は【admin のみ】
│  ⠿ Tenor II           [✏️][🗑️]│
│  ⠿ Bass               [✏️][🗑️]│
└──────────────────────────────────┘
ドラッグして表示順を変更できます。在団メンバーが所属しているパートは削除できません。 ← 【admin のみ表示】
```
...
- ⠿ ドラッグ&ドロップ → 表示順を入れ替え。変更があった行のみ`PATCH /parts/:id`を並列実行（失敗時は元の順序に戻し「並び替えに失敗しました」トースト）
...
- ⠿ ドラッグ&ドロップ → 表示順を入れ替え。変更があった行のみ`PATCH /expense-categories/:id`を並列実行（失敗時は元の順序に戻し「並び替えに失敗しました」トースト）
- 「追加」→ インライン入力欄（名前のみ）
- ✏️ → インライン編集
...
│  区分名       │ デフォルト会費│         │
├──────────────┼────────────┼──────────┤
│  ⠿ 社会人     │  ¥3,000    │ [✏️][🗑️]│ ← 並び替え（ドラッグハンドル）・編集・削除は【admin のみ】
│  ⠿ 学生       │  ¥1,500    │ [✏️][🗑️]│
└──────────────┴────────────┴──────────┘
ドラッグして表示順を変更できます。団員が割り当てられている区分は削除できません。削除前に区分を変更してください。 ← 【admin のみ表示】
```
...
- ⠿ ドラッグ&ドロップ → 表示順を入れ替え。変更があった行のみ`PATCH /member-types/:id`を並列実行（失敗時は元の順序に戻し「並び替えに失敗しました」トースト）
...
│  ⠿ ● 練習       [標準]      [✏️]    │ ← 標準区分は🗑️非表示
│  ⠿ ● 本番       [標準]      [✏️]    │
│  ⠿ ● 合宿                  [✏️][🗑️]│ ← 並び替え（ドラッグハンドル）・追加・編集・削除は【admin のみ】
├──────────────────────────────────────┤
│  区分を追加                            │ ← 【admin のみ表示】
│  [色選択][プリセット8色] [区分名] [追加]│
└──────────────────────────────────────┘
練習・本番などのシステム標準区分は削除できません。名前・色の変更は可能です。
ドラッグして表示順を変更できます。 ← 【admin のみ表示】
```

> `finance`ユーザーには一覧のみ表示され、並び替え・追加・編集・削除の操作UIと「ドラッグして表示順を変更できます。」の説明文は表示されない（閲覧専用）。「区分を追加」フォーム自体も表示されない。

#### インタラクション

- ⠿ ドラッグ&ドロップ → 表示順を入れ替え。変更があった行のみ`PATCH /event-categories/:id`を並列実行（失敗時は元の順序に戻しエラー表示）
...
- ここで設定したテンプレートが、見学申込の承認画面（[5.5](#55-見学申込-orgmembersapplicationsadmin)）で生成される紹介メール下書き・コピー用テキストに使われる

---

## 13. 資料ライブラリ

### 13.1 資料ライブラリ `/[org]/library`

**目的**: 団体規約・議事録・団員ガイド等の共有資料と、楽譜・本番・イベントに紐づく添付ファイルを、種別タブで横断して閲覧・管理する1画面に集約する。
...
│  資料 │ 楽譜 │ 本番 │ イベント                       │
├──────────────────────────────────────────────────────┤
│  すべて│規約・規則│議事録│団員ガイド│会計報告│その他 │
│  （「資料」タブ選択時のみ表示）                       │
├──────────────────────────────────────────────────────┤
│  📄 団体規約                          [団員限定] 🗑  │
│     bylaws_2026.pdf                  （admin のみ）  │
│  📄 第85回総会議事録                    [客演にも公開]│
│     minutes_20260601.pdf                             │
└──────────────────────────────────────────────────────┘
```

> 上部は「資料｜楽譜｜本番｜イベント」の主タブ。「資料」タブ選択時のみ、その下にカテゴリタブ（すべて／規約・規則／議事録／団員ガイド／会計報告／その他）が表示される。「楽譜」「本番」「イベント」タブではカテゴリタブの代わりに横断ファイル一覧（[13.3](#133-楽譜本番イベントタブ横断ファイル一覧)）を表示する。

#### 主タブ別の「＋追加」ボタン表示条件

| タブ               | ボタン表示条件                                                                                                                        | 押下時の遷移                     |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------- |
| 資料               | `admin`                                                                                                                               | 資料アップロードモーダル（13.2） |
| 楽譜/本番/イベント | `admin`/`tech`/`conductor`/`score`（3タブ共通の単一判定。楽譜タブのみ、選択後のモーダル内でPDF/MIDIそれぞれ更に細かい権限判定がある） | リソース選択モーダル（13.4）     |

#### 資料タブの各行の要素

| 要素                           | 表示条件                                           |
| ------------------------------ | -------------------------------------------------- |
| タイトル（ダウンロードリンク） | 全員（閲覧可能な資料のみ一覧に含まれる）           |
| 公開範囲バッジ                 | 全員（`管理者限定` / `団員限定` / `客演にも公開`） |
| 削除ボタン（🗑）                | `admin` のみ。行ホバー時に表示                     |

> 閲覧可能な範囲は `accessLevel` で決まる: `secret`=admin限定、`restricted`=member以上、`public`=guest以上。visitor は `accessLevel` を問わず資料タブ自体を利用できない（サイドバーにも「資料」項目が表示されない）。

**削除確認ダイアログ**（admin）

- 🗑 → タイトルを表示した確認ダイアログ → 「削除する」で `DELETE /documents/:id` を実行

---

### 13.2 資料アップロードモーダル（admin）

**目的**: PDF資料を新規登録する（資料タブ内モーダル）。
...
| ---------- | ---------------------------------------------------- | -------------------------------------- |
| タイトル   | テキスト                                             | ✅                                     |
| カテゴリ   | 選択（規約・規則/議事録/団員ガイド/会計報告/その他） | ✅（デフォルトは表示中のカテゴリタブ） |
| 公開範囲   | 選択（客演にも公開/団員限定/管理者限定）             | ✅（デフォルト: 団員限定）             |
| ファイル   | ファイル選択（PDFのみ）                              | ✅                                     |

#### インタラクション

- アップロード → プレサインドURL発行→R2へPUT→アップロード確定の3段階を内部で実行し、成功したら一覧を再取得してモーダルを閉じる
- 失敗時はモーダル内にエラーメッセージを表示し、モーダルは閉じない

---

### 13.3 楽譜/本番/イベントタブ（横断ファイル一覧）

**目的**: 各ドメイン画面（楽譜詳細・本番詳細・イベント詳細）に散らばる添付ファイルを、ファイル単位で横断的に確認し、対象の詳細画面へ素早く遷移する。
...
| 要素                           | 内容                                                                      |
| ------------------------------ | ------------------------------------------------------------------------- |
| タイトル（ダウンロードリンク） | 楽譜=楽譜タイトル、本番/イベント=ファイルのラベル                         |
| サブタイトル                   | 楽譜=ファイル種別（楽譜PDF/MIDI等）、本番/イベント=紐づく本番・イベント名 |
| 「詳細へ」リンク               | 対象の楽譜詳細 / 本番詳細 / イベント詳細ページへ遷移                      |

> この一覧はファイルの閲覧のみを担う。削除は行内では行わず、「詳細へ」から対象の詳細画面（既存のファイル管理UI）で行う。閲覧可否は種別ごとに既存画面と同じ判定（楽譜=購入記録/secret/visitorはfull_scoreのみ、本番=全団員閲覧可、イベント=招待対象またはadmin）。

---

### 13.4 リソース選択モーダル（楽譜/本番/イベント共通、ファイル管理権限保持者）

**目的**: 「＋追加」で対象の楽譜・本番・イベントを検索して選び、既存のファイル管理UIに橋渡しする。

#### インタラクション

- 検索欄に入力 → 300msデバウンス後に検索（楽譜はサーバー側のタイトル検索API、本番・イベントは全件取得してクライアント側でタイトル絞り込み）
- 一覧から1件クリック → モーダルを閉じ、選択した種別に応じたファイル管理モーダル（13.5）を開く

---

### 13.5 選択後のファイル管理モーダル

**目的**: リソース選択後、そのリソース専用の既存ファイル管理UIをそのまま呼び出す（新規UIは作らず既存コンポーネントを再利用）。

| 選択した種別 | 表示するモーダル                                                                   | 備考                                                                                                 |
| ------------ | ---------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| 楽譜         | 楽譜詳細の「ファイル管理モーダル」（[7.3](#73-楽譜詳細-orgscoresscoreid)と同一）   | 先に楽譜詳細・パート一覧を取得してから表示。取得中はスピナー、失敗時はエラーメッセージ＋閉じるボタン |
| 本番         | 本番詳細の[ファイルタブ](#ファイルタブ)と同一                                      | ファイル管理権限がない場合はアップロード欄を表示せず閲覧のみ                                         |
| イベント     | [イベント詳細](#62-イベント詳細出欠表-orgscheduleid)の添付ファイルセクションと同一 | 同上                                                                                                 |

- モーダルを閉じる → 横断ファイル一覧（13.3）を再取得

---

## 14. 通知

### 14.1 通知一覧 `/[org]/notifications`

**目的**: ヘッダーの通知ドロップダウン（直近8件のみ）に収まらない全通知を、ページネーション付きで確認する。ドロップダウンの「すべて見る」からのみ導線があり、サイドバーに常設ナビ項目は無い。
...
│    みなさんこんにちは。7月の練習日程を...          │
├──────────────────────────────────────────────────┤
│    見学申込がありました（見学 太郎）        5/28  │
└──────────────────────────────────────────────────┘
```

- 未読は太字＋左端に青いドット●＋薄い青背景、既読は通常のグレー表示（`NotificationBell`のドロップダウンと同じ見た目）
- 「すべて / 未読 / 既読」タブ（`ComposeModal`の送信先タブと同じ見た目）で絞り込み。タブ切り替え時はページを1に戻す
- 0件時は絞り込み状態に応じた空状態文言を表示（「通知はありません」/「未読の通知はありません」/「既読の通知はありません」）

#### インタラクション

- 行クリック → 未読なら既読化 → `link`先へ遷移
- 21件以上ある場合はページネーションを表示
- 既読から90日経過した通知は日次バッチで自動削除される（画面上に削除操作は無い）
...
flowchart TD
    ...
    subgraph tenant ["/:orgSlug/（認証後）"]
        ...
        subgraph libdetail ["資料ライブラリタブ"]
            LIB --> LIB_DOC[資料タブ]
            LIB --> LIB_SCORE[楽譜タブ]
            LIB --> LIB_CONCERT[本番タブ]
            LIB --> LIB_EVENT[イベントタブ]
        end
        LIB_DOC -. モーダル .-> LIBUP[資料アップロードモーダル]
        LIB_SCORE -. モーダル .-> LIBPICK[リソース選択モーダル]
        LIB_CONCERT -. モーダル .-> LIBPICK
        LIB_EVENT -. モーダル .-> LIBPICK
        LIBPICK -. モーダル .-> LIBFILES[ファイル管理モーダル]

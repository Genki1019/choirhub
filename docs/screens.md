...
- ログイン成功 → 所属団体数で分岐
  - 1団体 → `/{orgSlug}` へ直接リダイレクト
  - 複数団体 → `/select-org`（団体選択画面）へリダイレクト
...
**目的**: 複数の合唱団に所属するユーザーが、アクセスする団体を選ぶ

**表示条件**: ログイン済みユーザー全員がアクセス可能（1団体しか所属していない場合も URL 直打ちで来られる）

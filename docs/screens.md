...
- エラー（409以外） → 「送信に失敗しました。しばらくしてから再試行してください」

---

### 3.3b 団体作成申請 `/apply`

**目的**: ChoirHubのアカウントを持たない訪問者でも団体作成を申請できる、ログイン不要の単独ページ。トップページ（3.1）のCTAから遷移する。
...
**目的**: システム管理者が団体作成申請を確認・承認・却下する、および申請を経由せず団体を作成する
...
保留中の申請が0件の場合は「保留中の申請はありません」を表示する。
...
- 「承認」→ 編集後（未編集ならそのまま）のスラグを添えて `POST /auth/org-applications/:id/approve` を呼ぶ。成功すると団体・パート・招待メールが作成/送信され、一覧から該当行が消える。スラグが既存団体と重複している場合は409が返り「このスラグはすでに使用されています」を表示。スラグ形式が不正な場合（通常はボタンの無効化で防がれる）は400が返り「スラグの形式が正しくありません（英小文字・数字・ハイフン、2〜50文字）」を表示（いずれも一覧には残るので再編集して再試行できる）
